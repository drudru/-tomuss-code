...
def dateRegtest():
    ...
    global year_month_day, current_seconds
...
def filterRegtest():
    ...
    for tst in ('<4', '<=3', '>4', '=5', '>=5', '@o', '@<j', ':>3', ':~3',
                ...
                '?', '?<=4j',
                'undefined',
                u"@~è", contextual_case_sensitive and "@~J" or 'NO',
                u"@~=/", # Test if → is lost
                ):
        if Filter(tst, "", "").eval(c):
            bug("BUG3", tst)
    ...
    for cell_filters_result in (
        ...
        if Filter(filters[0], filters[1], filters[2]
                  ).eval(Cell(cell[0], cell[1], cell[2], cell[3], cell[4]
                              )) != result:
...
if not python_mode and str(5) != '5': # For translation to JS source
    float2str_bug
dateRegtest()
filterRegtest()
