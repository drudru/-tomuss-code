...
print "Enter YES to delete %s links to a strange file:" % len(bad_link),
if sys.stdin.readline().strip() == 'YES':
    for name in bad_link:
        os.unlink(name)

print "Enter YES to delete %s links to a strange year:" % len(bad_year),
if sys.stdin.readline().strip() == 'YES':
    for name in bad_year:
        os.unlink(name)

print "Enter YES to delete %s links to a strange semester:" % len(bad_semester),
if sys.stdin.readline().strip() == 'YES':
    for name in bad_semester:
        os.unlink(name)

print "Enter YES to delete %s links to non existing file:" % len(bad_file),
if sys.stdin.readline().strip() == 'YES':
